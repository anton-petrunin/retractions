...
it makes possible to extend a number of results from Hadamard manifolds to Riemannian manifolds of small size
...
Let us list few existence results which follow from the theorem, assuming that the space ${\spc{U}}$ is as above:
\begin{enumerate}[(a)]
...
\parit{Proof.}
Set $f_t=- \max\{\tfrac\pi2,\dist_{\gamma(t)}\}$.
Consider the map $\phi^t\:\alpha(a)\mapsto \alpha(t)$, where $\alpha$ is a $f_t$-gradient curve;
By \ref{prop:time-dependent}, if $\phi^t(p)$ is unique if it is defined.
Consider the function $\ell(t)\z=|\phi^t(p)-\gamma(t)|$.
By the definition of the flow it follows that $\ell'\le 0$ if $\ell> \tfrac\pi2$.
It follows that $\phi^t$ maps $\bar B[\gamma(a),\tfrac\pi2]$ to $\bar B[\gamma(t),\tfrac\pi2]$ for any $t$.

By $\CAT(1)$-comparison, given $\eps>0$ there is $\delta>0$ such that $f_t$ is $\eps$-concave in $ B(\gamma(t),\tfrac\pi2+\delta)$ for any $t$.
Applying the distance estimate (\ref{Distance estimate}) for $s=0$, we get that 
\[|\phi^t(p)-\phi^t(q)|\le |p-q|\cdot e^{\eps\cdot (t-a)}\]
for any $p,q\in \bar B[\gamma(a),\tfrac\pi2]$, $t\ge a$.
Since $\eps>0$ is arbitrary, (\ref{non-strict}) follows.
The proof of (\ref{strict}) is analogous.
To prove part (\ref{sharafutdinov}) it is sufficient to notice that $\phi^t(p)=p$ up to the moment when $|p-\gamma(t)|=\tfrac\pi2$.
\qeds

In addition we give the following sketch of a very direct proof;
a reader experienced with $\CAT(\kappa)$ comparison should be able to fill the details.

\parit{Stretch of an alternative proof.}
...
Fix a fine partition $a=t_0\z<\dots<t_n=b$ and set $q_i=\gamma(t_i)$.
...
Moreover if $T<\infty$ then $\alpha$ escapes from any closed set $K\subset \Omega$.
\end{thm}
...
Then $\alpha$ is a $f_t$-gradient curve if and only if 
...
If $(p',b)\in \Omega$, then we can repeating the the procedure, otherwise it escapes from any closed set in $\Omega$. 
\qeds
