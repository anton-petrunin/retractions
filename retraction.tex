...
for the definitions of ultrafilters, ultraproducts, and ultracompletions we refer to \cite{Wenger-1comp,guo-wenger,akp}. 
...
Let us list few existence results which hold for $1$-complemented $\CAT(1)$ %do we need it???
space ${\spc{U}}$:
\begin{enumerate}[(a)]
...
(See \cite{KS}, 
for the descriptions of the Dirichlet problem and \cite[Theorem 1.4]{guo-wenger} for the cited result).
\item The existence of a minimal integral $k$-current filling of any prescribed boundaryless $(k-1)$ integral current in ${\spc{U}}$. 
(See \cite{Ambrosio} for a discussion of Plateau's problem in  the theory of currents and \cite[Theorem 3.3]{Wenger-1comp}.)
...
(See \cite{LWplateau} for a discussion classical Plateau's problem for discs in metric spaces and \cite[Theorem 1.2]{guo-wenger}).
...
The latter contradicts existence of the strictly short retraction $\Psi$ provided by \ref{cor}.

\parbf{About the proofs.}
The proof of Proposition~\ref{thm:retraction:Phi} are done by using a simple trick together with a new tool which we call \emph{tractrix flow}.
The tractrix flow gives a family of maps $\phi_t$ for a given rectifiable curve $t\mapsto\gamma(t)$.
...
The trick is a combination of the spherical-join construction and the Reshetnyak gluing theorem. 
The Corollary \ref{cor} is proved using another trick with the spherical-join construction.
...
First  is the so called \emph{Sharafutdinov's retraction} \cite{sharafutdinov} --- a family of maps associated to a continuous family of convex sets (in our case these sets are the balls $B_t$). 
Second to the \emph{pursuer flow} introduced and studied by Stephanie Alexander, Richard Bishop, Robert Ghrist and Chanyoung Jun \cite{ABG,jun-thesis,jun,jun:grad}.
The proofs from any of these papers can be easily adapted to prove our proposition.

Let us also mention that time-dependent gradient flow was studied in \cite{jun-thesis,jun:grad} but only in the $\CAT(0)$ case, despite that $\CAT(\kappa)$ case is equally simple.
Gradient flow for time-independent function in $\CAT(\kappa)$ spaces were studied in \cite{lytchak-open-map} and \cite{ohta-palfia} and it is not hard to generalize their results to the time dependent flow.
In both cases we get a proof of the proposition.

In addition, let us indicate a direct proof of the proposition;
