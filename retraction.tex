...
The family $f_t$ is called \emph{locally uniformly upper continuous} if for any closed set $K$ and a semiopen interval $[a,b)$ such that $K\times[a,b)\subset \Omega$ the family of the restrictions $f_t|_K$ converge uniformly to $f_a|_K$ as $t\to a^+$.
A family of functions $f_t$ is called \emph{Lipschitz} if 
the function $x\mapsto f_t(x)$ is $L$-Lipschitz for some fixed $L$.
It is called \emph{locally uniformly Lipschitz} if each $(p_0,t_0)\in \Omega$ there is a neighborhood $\Omega'$ such that 
...
A family $f_t$ is called \emph{locally uniformly semiconcave} if for for each $(p_0,t_0)\in \Omega$ there is a neighborhood $\Omega'$ and $\lambda\in\RR$ such that the restriction of $f_t$ to $\Omega'$ is $\lambda$-concave. 
...
Suppose that $f_t$ is
\begin{enumerate}[(i)]
\item locally uniformly upper continuous,
\item locally uniformly Lipschitz,
\item locally uniformly semiconcave.
\end{enumerate}
